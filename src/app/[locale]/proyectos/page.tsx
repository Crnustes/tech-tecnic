...
const pageCopy = {
  es: {
    metaTitle: 'Proyectos | Tech Tecnic',
    metaDescription:
      'Portafolio de proyectos web, e-commerce, apps y automatizacion realizados por Tech Tecnic.',
    badge: 'Portafolio',
    title: 'Nuestros proyectos',
    subtitle:
      'Los mismos casos de exito que ves en el home, ahora con detalle y el repositorio del chatbot WhatsApp con IA.',
    filterLabel: 'Filtrar:',
    allLabel: 'Todos',
  ...
  en: {
    ...
    title: 'Our projects',
    subtitle:
      'The same success stories you see on the home page, now with details and the WhatsApp AI chatbot repository.',
    filterLabel: 'Filter:',
    allLabel: 'All',
...
const translateTag = (tag: string, locale: SupportedLocale) => {
  const mapped = tagTranslations[tag]
  if (!mapped) return tag
  return mapped[locale]
}

const translateCategory = (category: string, locale: SupportedLocale) => {
...
export default async function ProyectosPage({
  ...
  const categories = [
    copy.allLabel,
    ...Array.from(new Set(localizedProjects.map(project => project.categoryLabel))),
  ]
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mb-12 text-gray-300">
          <div className="flex items-center gap-2 mr-2 text-gray-400">
            ...
            <span className="text-sm font-medium">{copy.filterLabel}</span>
          </div>
          {categories.map(cat => (
            <span
              key={cat}
